...
export default function Home() {
  ...
  async function load({ silent = false } = {}) {
    ...
    setError(null);
    try {
      const [data, library] = await Promise.all([api.getComics(), api.getLibraryStatus()]);
      setComics(data.files || []);
      setSourceCategories((library.sources || []).map((source) => source.category).filter(Boolean));
    } catch (err) {
      setError(err);
    } finally {
  ...
  useEffect(() => {
    let cancelled = false;
    load();
    api.syncLibrarySources(false)
      .then(() => { if (!cancelled) load({ silent: true }); })
      .catch(() => {});
  ...
  const filtered = useMemo(() => {
    ...
    return comics.filter((comic) => {
      ...
      const haystack = `${comic.name} ${comic.path || ''} ${comicCategory}`.toLocaleLowerCase('pt-BR');
      ...
      <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 sm:py-12 lg:px-8">
        <section className="mb-9">
          <div className="mb-7 flex flex-wrap items-end justify-between gap-4">
            ...
            {!loading && !error && (
              <p className="text-sm font-medium text-zinc-600">{comics.length} {comics.length === 1 ? 'HQ' : 'HQs'}</p>
            )}
          </div>
          ...
          {!loading && !error && categories.length > 0 && (
            <div className="mt-5 flex flex-wrap gap-2">
              <button
                ...
                className={`rounded-full border px-4 py-2 text-xs font-bold transition ${category === 'Todas' ? 'border-red-500 bg-red-500/15 text-red-300' : 'border-white/10 bg-white/[0.03] text-zinc-400 hover:bg-white/[0.07] hover:text-zinc-200'}`}
              >
                Todas <span className="ml-1 text-[10px] opacity-60">{comics.length}</span>
              </button>
              {categories.map(([name, count]) => (
                <button
                  ...
                  className={`rounded-full border px-4 py-2 text-xs font-bold transition ${category === name ? 'border-red-500 bg-red-500/15 text-red-300' : 'border-white/10 bg-white/[0.03] text-zinc-400 hover:bg-white/[0.07] hover:text-zinc-200'}`}
                >
